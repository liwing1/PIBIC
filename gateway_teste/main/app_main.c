...
EventGroupHandle_t esp32_event_group;
   ...
   for(;;) {
      if ( xSemaphoreTake(xMutex, (TickType_t) 0xFFFFFFFF) == 1 ){
         ...
         while(lora_received()) {
            ...
            printf("Received: %s\n", lora_rx_buf);
            lora_receive();
            if(strlen((char*)lora_rx_buf) <= 12) xQueueSend(rxQueue, (void*)lora_rx_buf, (TickType_t) 0);
            xEventGroupSetBits(esp32_event_group, MQTT_INITIATE_PUBLISH_BIT | MQTT_PUBLISHED_BIT);
         }
...
void lora_gw_send(void *p)
{
   if ( xSemaphoreTake(xMutex, (TickType_t) 0xFFFFFFFF) == 1 ){
      ...
      printf("LORA GW SENT: %s\n", (char*)p);
      SayHello( &I2CDisplay, p );
...
void app_main()
{
   ...
   xTaskCreate(&lora_gw_receive,"lora_gw_receive", 2048, NULL  , 5, NULL);

   xTaskCreate(&mqtt_tsk,       "mqtt_task"      , 2048, NULL  , 4, NULL);
}